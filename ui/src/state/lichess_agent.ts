...
const Lichess_NextAllowed_Fetch = '.linechess.$rate_limit.lichess.next_allowed_fetch'
let nextAllowedFetchAt = () => {
    let res = parseInt(localStorage.getItem(Lichess_NextAllowed_Fetch)!)

    return res ?? 0
}
const setNextAllowedFetch = () => {
    let nextAllowedFetchAt = Date.now() + 60_000
    localStorage.setItem(Lichess_NextAllowed_Fetch, `${nextAllowedFetchAt}`)
}

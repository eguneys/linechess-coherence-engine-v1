...
export class QPJ_Manager<T> {
    ...
    constructor(private do_work: (username: string) => Promise<T[]>) {
        this.jobs = []
    ...
    async run() {
        ...
        while (this.jobs.length > 0) {
            ...
            try {
                ...
                if (job) {
                    let res = await this.do_work(job.username)
                    job.priority = 0
                    job.last_checked_ms = Date.now()
...
const effective_priority = <T>(a: QueryPlayerJob<T>) => a.priority + (Date.now() - a.last_checked_ms) / HOUR_MS
